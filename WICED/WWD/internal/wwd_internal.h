...
#define WWD_IOCTL_LOG_SIZE 128
#define WWD_IOVAR_STRING_SIZE 48

#define WWD_IOCTL_LOG_ADD(x) wwd_ioctl_log_add(x)
#define WWD_IOVAR_LOG_ADD(x) wwd_iovar_log_add(x)
#define WWD_IOCTL_LOG_ADD_EVENT(x, y) wwd_ioctl_log_add_event(x, y)
...
typedef struct
{
    ...
    char iovar[WWD_IOVAR_STRING_SIZE];
} wwd_ioctl_log_t;

void wwd_ioctl_log_add(uint32_t cmd);
void wwd_iovar_log_add(const char *iovar);
void wwd_ioctl_log_add_event(uint32_t cmd, uint16_t flag);
#else
#define WWD_IOCTL_LOG_ADD(x)
#define WWD_IOVAR_LOG_ADD(x)
#define WWD_IOCTL_LOG_ADD_EVENT(x, y)

...
void wwd_wifi_peek( uint32_t address, uint8_t register_length, /*@out@*/ uint8_t* value )
{
    ...
    wwd_bus_read_backplane_value( address, register_length, value );
...
void wwd_wifi_poke( uint32_t address, uint8_t register_length, uint32_t value )
{
    ...
    wwd_bus_write_backplane_value( address, register_length, value );
...
{
    strcpy(wwd_ioctl_log[wwd_ioctl_log_index % WWD_IOCTL_LOG_SIZE].iovar, iovar);
}
...
wwd_result_t wwd_ioctl_print(void)
{
    ...
    for(i = 0; i < WWD_IOCTL_LOG_SIZE; i++)
    {
        ...
        {
            WPRINT_MACRO(("E %lu %x\n", wwd_ioctl_log[i].ioct_log, wwd_ioctl_log[i].flag));
        }
        else if(wwd_ioctl_log[i].ioct_log == WLC_SET_VAR ||
           wwd_ioctl_log[i].ioct_log == WLC_GET_VAR)
        {
            WPRINT_MACRO(("%lu %s\n", wwd_ioctl_log[i].ioct_log, wwd_ioctl_log[i].iovar));
        }

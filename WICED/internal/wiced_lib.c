...
{
    uint8_t digits_left;
    char buffer[sizeof(ULLONG_MAX_STR) + 1];  /* Buffer for storing ULLONG_MAX with +1 for the sign */
...
{
    uint8_t digits_left;
    char buffer[sizeof(ULONG_MAX_STR) + 1]; /* Buffer for ULONG_MAX with +1 for storing the sign */
    max_length = (uint8_t) MIN( max_length, sizeof( buffer ) );
    digits_left = max_length;
    while ( ( value != 0 ) && ( digits_left != 0 ) )
    {
